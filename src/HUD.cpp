...
HUD::HUD(Risorse *src){
    ...
    lifebar_active.setPosition(src->getLength()/2-200, 20);
    ...
    lifebar_back.setPosition(src->getLength()/2, 20);
    ...
    fuelbar_active.setPosition(src->getLength()/2, 20);
    ...
    fuelbar_back.setPosition(src->getLength()/2, 20);
    fuelbar_back.setScale(0.5, 0.5);
    //pausa = Pulsante(Vector2f(1200,30), src, 21, 0.35);
}

void HUD::gestisci(int point, float life, float fuel){
    ...
    src->getWindow()->draw(this->life);
    src->getWindow()->draw(this->fuel);
